...
- check if the spelling of the webpack names match with the import in the template.
...
#### Adding URL Route
All that is left is to add URL rules, so that the new functions are called instead of the ones from the base implementation. In `invenio_theme_tugraz/ext.py` import the previously defined function and inside the `init_app` function add:
